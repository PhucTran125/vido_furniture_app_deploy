        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-0 text-center divide-y md:divide-y-0 md:divide-x divide-white/10">
          {highlights.map((item, index) => (
            <Link
              key={index}
              href={item.href}
              className="group flex flex-col items-center pt-8 md:pt-0 px-6 cursor-pointer"
            >
              {/* Icon */}
              ...
              {/* Description */}
              <p className="text-gray-300 font-sans text-sm transition-opacity duration-300 group-hover:opacity-100 opacity-80 mb-5">
                {item.description}
              </p>
